...
:w !sudo tee % > /dev/null -> 以 sudo 方式保存文件 
...
# 标签 (tag)
:ta -> 跳转入标签 (jump to tag)
:ts -> 显示匹配标签，并且跳转入某个标签 (list matching tags and select one to jump)
:tags -> 显示所有标签 (print tag list)
...
# 全局 (global) (find the match pater and execute a command)
