...
tags:
  ...
  - CenterOS
categories: Docker
translate_title: docker-learning-notes
...
目录结构：
```bash :DockerFiles line_number:false
 DockerFiles
|--MySQL
|  |--DockerFile
|--PHP-FPM
...
```
### MySQL DockerFile：
```bash :MySQL/DockerFile line_number:false
FROM mysql:latest
...
### PHP-FPM DockerFile：
```bash :PHP-FPM/DockerFile line_number:false
FROM php:7.2-fpm
MAINTAINER learner.hui "learner.hui@gmail.com"
...
### Nginx DockerFile：
```bash :Nginx/DockerFile line_number:false
FROM nginx:alpine
MAINTAINER learner.hui "learner.hui@gmail.com"
...
### Redis DockerFile
```bash :Redis/DockerFile line_number:false
FROM redis:latest
MAINTAINER learner.hui "learner.hui@gmail.com"
# set timezome
ENV TZ=Asia/Shanghai
...
### docker-compose.yml
```bash :docker-compose.yml line_number:false
version: '1.0'
services:
  php-fpm:
    build: ./php/
